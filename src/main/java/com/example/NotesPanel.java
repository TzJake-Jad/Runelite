...
public class NotesPanel extends PluginPanel
{
	...
	private void saveNotes()
	{
		...
		for (NoteItem note : notes)
		{
			// Format: "1|text" (checked) or "0|text" (unchecked)
			// Newlines within text are replaced with a space since we use \n as record separator
	...
	private void loadNotes()
	{
		...
		if (data != null && !data.isEmpty())
		{
			...
			{
				// Minimum valid record: "0|x"
				if (line.length() < 3)
				{
					continue;
				}
				boolean checked = line.charAt(0) == '1';
				String text = line.substring(2); // skip "0|" / "1|"
				if (!text.isEmpty())
				{
